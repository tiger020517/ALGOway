...
// sets the compare function less() for maxheap, more() for minheap. max힙일떄 less, min힙일때 more...
...
void grow(heap p, int key) {
	...
	if(p->N+1 >= p->capacity) { reserve(p, p->capacity*2); }
...
void sink(heap p, int k) {
	...
	while (k*2 <= p->N){
		int child = k*2;
		if( child + 1 <= p->N && p->comp(p, child, child+1)) child++;
		if(p->comp(p,child,k)) break;
		swap(p, child, k);
...
*/
    ...
    bool isLeftOrdered = (left > p->N) || heapOrderedAt(p, left);
    if (!isLeftOrdered) return false;

    bool isRightOrdered = (right > p->N) || heapOrderedAt(p, right);
    return isRightOrdered;
...
void heapify2(heap p) { // I don't know why but there are an error that it couldn't heapify because of something when use z... -KJH-
...
void growN(heap p, int count, bool heapOrdered) {
	...
	int Max=p->nodes[1];
	for(int i =1; i<=p->N; i++){
		if(p->nodes[i] > Max) Max = p->nodes[i];
	}

	int newkeys [count] = {};

	randomN( Max , count , newkeys);
	for(int i = 0; i < count; i++){
		heapOrdered ? grow(p, newkeys[i]) : growCBT(p, newkeys[i]);
	}
    ...
    int max = empty(p) ? 0 : maximum(p);

    int* keys = new int[count];

    randomN(max + 1, count, keys);

    void (*growfuncpointer)(heap h, int key) = heapOrdered ? grow : growCBT;

    for (int i = 0; i < count; i++) {
        growfuncpointer(p, keys[i]);

		#ifdef DEBUG
        cout << "After inserting " << keys[i] << ": ";
        for (int j = 1; j <= p->N; j++) {
            cout << p->nodes[j] << " ";
        }
        cout << endl;
...
 randomN()을 호출하여 [(max + 1)…(max + count)] 범위 내 임의의 키를 가져옵니다.
...
void trimN(heap p, int count, bool heapOrdered) {
    ...
    void(*trimfuncpointer)(heap h) = heapOrdered ? trim : trimCBT;
