...
// Given a binary tree, its nodes are saved in vec by level.
void treeprint_levelorder(tree root, int level, vector<int>& vec) {
	DPRINT(cout << ">treeprint_levelorder vec lv=" << level << endl;);
	if (root == nullptr) return;
	if (level == 1) 
		vec.push_back(root->key);
	else {
		treeprint_levelorder(root->left, level - 1, vec);
		treeprint_levelorder(root->right, level - 1, vec);
	}
	DPRINT(cout << "<treeprint_levelorder vec lv=" << level << endl;);
}
...
void treeprint_levelorder_tasty(tree root) {
	...
	for (int i = 1; i <= 3; i++) {
		vector<int> vec;
		treeprint_levelorder(root, i, vec);
		cout << "[" << i << "] ";
		show_vector(vec);
		cout << endl;
	}
